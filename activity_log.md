...
### [DONE] azalea 2026-06-29 全章のステップにper-stepチェックボックスを付与（907→1860）＋難易度remap保全
- ユーザー要望: §5-3のように各作業ステップにチェックボックスを紐づけ（全章一括変換を選択）。
- 実装: ol.steps直下で未チェックの953ステップに <ul class="checklist"><li>要約ラベル</li></ul> をネスト挿入。ラベル＋難易度はマルチエージェント(21章並列)で生成。
- データ保全: tid=章/位置 のため、既存907の難易度(GN_TASKDIFF)を新indexへremapで全保全(hard202→新hard297=既存202+新95)。担当/進捗は位置ズレでリセット前提(ユーザー承諾済)。
- 安全策: html.parser(DOM正確)で検出/オフセット取得。自作walkerのオフセットバグ(GN_TASKDIFF差替で挿入後再マッチせず破損)を修正。新ファイル検証→本体反映。
- 検証(ブラウザ実機): ul.checklist>li=1860 / GN_TASKDIFF=1860 / tk-check=1860 / tk-diff-hard=297 / タグ収支OK / §5-4-1先頭ラベル描画OK。
- 注: 旧907のチェック済み/担当はindexズレで再割当が必要(リセット)。
